...
class AbilityHandSerialClient:
    def __init__(self, hand_addr=0x50, baudrate=460800, noprint=0):
        ...
        self.tPos = np.array([15.0, 15.0, 15.0, 15.0, 15.0, -15.0], dtype=np.float32)
            ...
            if p:
                ...
                try:
                    self.ser = serial.Serial(
                        serialport[0], self.baudrate, timeout=0, write_timeout=1
                    )
                    if self.suppress_print_messages == 0:
                        print("connected!")
                    break
    ...
    def __readloop(self):
        ...
        while self.continue_reading == True:
            ...
            if len(nb) != 0:
                ...
                for b in npbytes:
                    pld, stuff_buffer, bidx, pld_valid = unstuff_PPP_stream_Cstyle_fast(
                        b, stuff_buffer, bidx
                    )
                    if (
    ...
    def writePos(self):
        msg = farr_to_abh_frame(
            self.serial_address, self.tPos * 32767 / 150, 0x10 + self.reply_mode - 1
        )
        self.num_writes = self.num_writes + 1
        stuffed_payload = PPP_stuff(bytearray(msg))
        self.ser.write(stuffed_payload)
    ...
    def writeVelocity(self):
        msg = farr_to_abh_frame(
            self.serial_address,
            self.tVelocity * 32767 / 3000,
            0x20 + self.reply_mode - 1,
        )
        self.num_writes = self.num_writes + 1
        stuffed_payload = PPP_stuff(bytearray(msg))
        self.ser.write(stuffed_payload)
    ...
    def writeCurrent(self):
        msg = farr_to_abh_frame(
            self.serial_address,
            self.tCurrent / self.currentConversionRatio,
            0x30 + self.reply_mode - 1,
        )
        self.num_writes = self.num_writes + 1
        stuffed_payload = PPP_stuff(bytearray(msg))
        self.ser.write(stuffed_payload)
    ...
    def writeVoltageDuty(self):
        msg = farr_to_abh_frame(
            self.serial_address, self.tVoltageDuty * 3546, 0x40 + self.reply_mode - 1
        )
        self.num_writes = self.num_writes + 1
        stuffed_payload = PPP_stuff(bytearray(msg))
        self.ser.write(stuffed_payload)
